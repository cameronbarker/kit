...
class Kit::BriefCLITest < Minitest::Test
  ROOT = File.expand_path("../../..", __dir__)
  RUBY = RbConfig.ruby

  def setup
    @tmpdir = Dir.mktmpdir("kit-brief-cli-test-")
  ...
  def test_brief_json_reads_temp_vault_and_writes_artifact
    result = run_kit("brief", "--json", "--me", "Cameron")

    assert_equal 0, result[:status], result[:stderr]
    assert_empty result[:stderr]

    payload = JSON.parse(result[:stdout])
    assert_equal "kit_brief", payload["kind"]
    assert_equal @vault, payload["vault"]
    assert_equal "Cameron", payload.dig("filters", "me")
    assert_equal ["mine-done"], ids(payload.dig("sections", "what_moved"))
    assert_equal ["mine-open"], ids(payload.dig("sections", "waiting_on_me"))
    assert_equal ["other-open"], ids(payload.dig("sections", "waiting_on_others"))
    assert_equal ["decision-open"], ids(payload.dig("sections", "decisions_needed"))
    assert_equal ["loop-open"], ids(payload.dig("sections", "open_loops"))
    assert_equal ["mine-possible", "loop-possible"], ids(payload.dig("sections", "needs_review"))
    assert File.file?(payload["artifact_path"])
    assert_includes File.read(payload["artifact_path"]), "# Weekly Brief:"
  ...
  def test_human_output_includes_artifact_and_sections
    ...
    assert_equal 0, result[:status], result[:stderr]
    assert_includes result[:stdout], "Kit brief"
    assert_includes result[:stdout], "note:"
    assert_includes result[:stdout], "What moved:"
    assert_includes result[:stdout], "Send the rollout note."
    assert_includes result[:stdout], "Needs review:"
  end

  def test_brief_uses_env_defaults_when_flags_absent
    result = run_kit_without_vault("brief", "--json", env: { "KIT_VAULT" => @vault, "KIT_ME" => "Cameron" })
  ...
  def test_brief_warns_without_me
    result = run_kit("brief", "--json")

    assert_equal 0, result[:status], result[:stderr]
    payload = JSON.parse(result[:stdout])
    assert_empty payload.dig("sections", "waiting_on_me")
    assert_includes payload["warnings"], "Missing --me or KIT_ME; waiting-on-me classification is disabled."
  end

  def test_brief_rejects_unexpected_arguments
    result = run_kit("brief", "weekly")

    assert_equal 1, result[:status]
    assert_empty result[:stdout]
    assert_includes result[:stderr], "Error: unexpected arguments: weekly"
  end
